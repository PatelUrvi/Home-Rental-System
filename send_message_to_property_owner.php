    ...
    <!-- ##### Breadcumb Area End ##### -->
     <section class="listings-content-wrapper section-padding-100">
         ...
         <div class="row">
          ...
          <div class="col-10">
         <?php
         $find = mysqli_query($con, "select * from tbl_tenants where emailid = '".$_SESSION['email']."'");
         while ($find2 = mysqli_fetch_assoc($find))
         {
             $find1 = mysqli_query($con, "select * from tbl_send_message_from_owner where owner_emailid = '".$id."' and emailid = '".$_SESSION['email']."'");
         ...
         ?>
               <?php
              $find4 = mysqli_query($con, "select * from tbl_property_owner where emailid = '".$id."'");
         while ($find5 = mysqli_fetch_assoc($find4))
         {
             $find6 = mysqli_query($con, "select * from tbl_send_message_from_tenant where emailid = '".$id."' and tenant_emailid = '".$_SESSION['email']."'");
...
{ 
            ...
            $row1 = mysqli_fetch_assoc($q1);
             $message=$_POST['mess'];
             $q = "insert into `tbl_send_message_from_owner`(`name`,`emailid`,`owner_emailid`,`contactno`,`message_body`,`date`) VALUES ('".$row1['name']."','".$_SESSION['email']."','".$id."','".$row1['contactno']."','".$message."','". date("y-m-d H:i:sa")."')";
