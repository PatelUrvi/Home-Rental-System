    ...
    if(isset($_POST['submit']))
        {
            ...
            if($new==$retype)
            {
                if($old!='' && $new!='' && $retype!='')
                {
                    ...
                    $sql1 = "SELECT password FROM tbl_user WHERE emailid='" . $_SESSION['emailid'] . "' && user_type='property owner'";
                    $query = $con->query($sql1);
                    $pass = $query->fetch_assoc();
                    
                    if($old == $pass['password'])
                    {
                        $sql = "UPDATE tbl_user SET password='" . $new . "' WHERE emailid= '".$_SESSION['emailid']."' && user_type='property owner' ";
                        $query = $con->query($sql);
